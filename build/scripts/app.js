...
function imagenSeleccionar() {
  // Espera a que el documento esté completamente cargado

  // Selecciona la imagen principal
  const playeraSeleccionada = document.querySelector(".playeraSeleccionada");

  // Selecciona todas las imágenes pequeñas
  const miniaturas = document.querySelectorAll(".PlayeraImagenes img");

  // Agrega un event listener a cada miniatura
  miniaturas.forEach((img) => {
    img.addEventListener("click", () => {
      // Cambia el src de la imagen principal al de la miniatura clickeada
      playeraSeleccionada.src = img.src;
    });
  });
}
